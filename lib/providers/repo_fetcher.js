/** @module lib/providers/repo_fetcher */
...
function filterRepositories(repositories, options) {
  return repositories.filter(r => repositoryFilter(r, options));
}
...
async function fetchRepos(requestOptions, responseConverter, options) {
  /**
     * Gets one page of repositories.
     * @param {number} pageNumber - The page number.
     * @returns {array} The repositories.
     */
  async function getRepositories(pageNumber) {
    const requestOptionsClone = Object.assign({}, requestOptions);
    if (pageNumber > 1) {
      requestOptionsClone.path = `${requestOptionsClone.path}?page=${pageNumber}`;
    }

    logger.log(`Fetching repository info ${requestOptionsClone.path}`);
    const response = await httpsPromise(requestOptionsClone);
    const repositories = responseConverter(response);
    return repositories;
  }

  /**
     * Gets all repositories recursively.
     * @param {array} existingRepositories - The existing repositories.
  ...
  async function getRepositoriesRecursive(existingRepositories, pageNumber) {
    ...
    const isLastPage = !repositories || !repositories.length || repositories.length <= 0;
