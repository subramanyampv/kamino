 ...
 * "notifications_url": ".../repos/ngeor/clone-all/notifications{?since,all,participating}",
...
function getRepositories(options) {
    ...
    serverDefaults.path = '/users/' + options.username + '/repos';
    return repoFetcher(serverDefaults, response => JSON.parse(response), options);
}
