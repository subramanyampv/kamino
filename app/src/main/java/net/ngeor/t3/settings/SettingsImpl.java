...
public class SettingsImpl implements Settings, Serializable {
    ...
    public SettingsImpl(int rows, int cols, boolean invisibleMode, List<PlayerDefinition> playerDefinitions) {
        this.rows = rows;
        this.cols = cols;
    ...
    public boolean equals(Object o) {
        ...
        if (rows != settings.rows) return false;
        if (cols != settings.cols) return false;
        if (invisibleMode != settings.invisibleMode) return false;
        return playerDefinitions.equals(settings.playerDefinitions);
    }
