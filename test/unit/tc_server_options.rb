...
class TestServerOptions < Test::Unit::TestCase
  def test_create
    ...
    assert_equal("ServerOptions {provider: , username: , password: }", server_options.to_s)
  end
  ...
  def test_with_properties
    ...
    assert_equal('ServerOptions {provider: GitHub, username: ngeor, password: secret}', server_options.to_s)
  end
end
